...
1.  We need to change any characters, into dummy variables that are numerics, because scalers and imputaters do not recognize characters. In this case, `Age_group` is an ordinal category, so we will use `OrdinalEncoder` from `Scikit-learn`, specifically in `preprocessing` which we imported as `pproc`.
