...
# Exploratory Data Analysis in Python - Correlate Like a Data Master {.unnumbered}
...
# Drop NAs from observed Sap_Flow values
X_Obsv = X.dropna()

# Predicted values
ObsvVal = X_Obsv.Sap_Flow

X_max = ObsvVal.max() # X max
X_min = ObsvVal.min() # X min

# New dataframe from predicted values
X_Pred = pd.DataFrame(model_result.fittedvalues)

# Name the column for predicted values
X_Pred.columns = ["Pred"]

# Add the predicted values to the Y value dataframe
X_Comp = pd.concat([X_Obsv, X_Pred], axis = 1)

# Subplots
fig, (ax1, ax2) = plt.subplots(ncols = 2, nrows = 1)

# Regression plot between Sap_Flow and pLWP
sns.regplot(data = data, x = "Sap_Flow", y = "pLWP", ax = ax1)

# Set regression plot title
ax1.set_title("Linear regression")

# Scatterplot with linear predicted vs observed pLWP values
pred_plot = sns.scatterplot(x = X_Comp.Pred, y = X_Comp.Sap_Flow, alpha = 0.5, s = 50, ax = ax2)

# Set the length for a line segment
X_ref = Y_ref = np.linspace(X_min, X_max, 100)

# Linesegment representing a slope of 1
pred_plot.plot(X_ref, X_ref, color = 'red', linewidth = 2)

# Set pred vs. obsv plot title
pred_plot.set_title("Predicted vs. observed")

# Set x and y axes titles and limits
pred_plot.set(xlabel = "Predicted value of Sap_Flow", ylabel = "Observed value of Sap_Flow",
